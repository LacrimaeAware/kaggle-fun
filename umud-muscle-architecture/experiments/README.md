...
## exp26 - scale-cue pseudo-label export (`exp26_scale_cue_pseudolabels.py`)  [diagnostic/training prep]

Question: the current scale router is hand-built. Can we turn its trusted detections into
machine-learning-ready labels for a future learned tick/ruler/bar detector while staying in the
automated/no-oracle mode?

Implementation: export weak masks/boxes/overlays only from the production-accepted router path for
each target image. Raw detectors are not allowed to label out of context because the first run proved
they over-fire badly (for example, the raw 3 cm bar detector fired on 103 rows instead of the four
fallback rows it was designed for). The corrected exporter labels only accepted cues plus the narrow
bottom-scale-bar path on router-`none` rows. Outputs:
...
| item | value |
| --- | ---: |
| Target images scanned | 309 |
| Images with any code-generated cue label | 299 |
| Label rows | 299 |
| bottom_ticks labels | 59 |
| right_ruler_5mm labels | 87 |
| left_ruler_1cm labels | 50 |
| png_left_ruler labels | 58 |
| family_b_signature labels | 41 |
| bottom_scale_bar_3cm labels | 4 |

Read: this is the concrete bridge from deterministic scale logic to a learned scale-cue model. It is
not human annotation and not a submission. The host discussion makes human target labels a
declared-external-data option, but exp26 intentionally avoids that path; keep these masks/boxes fully
generated by code if this branch is presented as automated/no-oracle.

## exp27 - local public/external asset inventory (`exp27_external_asset_inventory.py`)  [bookkeeping]

Question: are the external/public assets hypothetical, or are they already present locally and usable
for supervised modeling?

Implementation: inventory the local `data/` tree, count image/mask pairs, and record public docs,
licenses/policies, benchmark files, target files, and weight files. Output:
...
| asset | count |
| --- | ---: |
| public image/mask pairs, target A | 1048 |
| public image/mask pairs, target B | 2761 |
| public reference benchmark images | 35 |
| competition target images | 309 |
| public pretrained weight files found locally | 1 |

Read: the supervised/public-data path is real and local. The safest next modeling branch is either
(a) retrain/ensemble the existing segmentation models with these public labels and declared external
assets, or (b) train a scale-cue detector from exp26's code-generated weak labels. These are genuine
image-understanding improvements, unlike output-mean tricks.

## exp28 - weak-label scale-cue segmenter (`exp28_train_scale_cue_segmenter.py`)  [training prep]

Question: can the exp26 cue-label artifact feed an actual learned detector path, rather than just a
CSV?
...
| item | value |
| --- | ---: |
| smoke train/val items | 19/5 |
| smoke device | CPU |
| smoke epochs | 1 |
| smoke best weak-label val Dice | 0.0015 |

Read: the smoke test only verifies the data/model path. The low one-epoch Dice is expected and is
...
## exp30 - host-protocol measurement alignment (`UMUD_MT_MODE`, `UMUD_FL_FRAGMENT_MODE`)  [candidate]

Question: does the production measurement code disagree with the host's public description of how
the hidden values were created?

Implementation: add two isolated measurement toggles in `segment_then_measure.py`.

- `UMUD_FL_FRAGMENT_MODE=min_extrap_top3` ranks fragment spans by visible-length / extrapolated-span
  and takes the median of the top 3, approximating the host's "pick low-extrapolation visible
  structures" instruction.
- `UMUD_MT_MODE=vertical_3` measures the straight vertical gap at left/middle/right and averages,
  approximating the host's global-image straight-line thickness procedure.
- `local_infer.py` now accepts `UMUD_LOCAL_OUT` and `UMUD_LOCAL_DEBUG_OUT`, so candidate runs do not
  overwrite the restored `0.61918` baseline.

Benchmark sanity check on the 35 expert-reference images, with true scale and `UMUD_FL_IDENTITY_BLEND=0`:

| variant | overall | PA | FL | MT |
| --- | ---: | ---: | ---: | ---: |
| baseline: median FL, perpendicular-center MT | 0.2274 | 0.1498 | 0.3528 | 0.1795 |
| FL low-extrapolation top-3 only | 0.2320 | 0.1498 | 0.3668 | 0.1795 |
...
- Scale-cue pseudo-label export is now built in `exp26`: 299/309 target images get a
  code-generated cue label from the trusted router path or the narrow visible-bar fallback. This is
