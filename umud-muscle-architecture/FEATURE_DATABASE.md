...
The active next lever is segmentation retraining (EXP59). If better masks improve public score, continue with model-quality work. If segmentation fails, the next durable work is not another broad scale patch; it is either a better trusted pixel-span detector for scale assets or a higher-quality target validation set.

## Next Tests To Add To This Database

1. Run the EXP59 GPU segmentation matrix and record each public score.
