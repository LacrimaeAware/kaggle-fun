...
# EnergyType int (obs) -> cost token. Rainbow is universal; Team Rocket is Psychic/Darkness only.
EN = {0: "C", 1: "G", 2: "R", 3: "W", 4: "L", 5: "P", 6: "F", 7: "D", 8: "M", 9: "N", 10: "*", 11: "PD"}
# OptionType
...
def energy_shortfall(cost: list[str], attached: list[str]) -> int:
    ...
    for c in cost:
        if c == "C":
            ...
            avail[c] -= 1
        elif c in ("P", "D") and avail.get("PD", 0) > 0:
            avail["PD"] -= 1
        elif wild > 0:
            wild -= 1
        else:
...
def _attached_letters(pkmn: dict) -> list[str]:
    raw = pkmn.get("energies")
    if raw is not None:
        return [EN.get(e, "C") for e in (raw or [])]
    out = []
    for card in (pkmn.get("energyCards") or []):
        cid = card.get("id") if isinstance(card, dict) else card
        row = cf(cid)
        ty = str(row.get("ty", "") or "")
        # Special energies in this project are mostly Psychic/Colorless.  Unknown
        # cards remain colorless rather than being hallucinated as a wildcard.
        out.append(ty[0] if ty else "C")
    return out
...
def encode_state(obs: dict) -> dict:
    ...
    f["turn_action_count"] = float(cur.get("turnActionCount", 0) or 0)
    f["bench_space"] = max(0.0, float(me.get("benchMax", 5) or 5) - f["my_bench"])
    board_ids = [p.get("id") for p in ([myA] if myA else []) + (me.get("bench") or []) if p]
    f["abra_in_play"] = float(sum(cid == 741 for cid in board_ids))
    f["kadabra_in_play"] = float(sum(cid == 742 for cid in board_ids))
    f["alakazam_in_play"] = float(sum(cid == 743 for cid in board_ids))
    f["dunsparce_in_play"] = float(sum(cid in (65, 305) for cid in board_ids))
    f["dudunsparce_in_play"] = float(sum(cid == 66 for cid in board_ids))
    ...
    if myA:
        ...
        if myA.get("id") == 743 and f["active_n_energy"] >= 1:
            # Powerful Hand places 2 damage counters per card; no weakness/resistance.
            ph_damage = 20.0 * f["hand_size"]
            f["active_affordable_dmg"] = ph_damage
            f["active_can_attack_now"] = 1.0
            f["active_energy_short"] = 0.0
            f["can_ko_opp_now"] = 1.0 if (opA and ph_damage >= opA.get("hp", 0) and ph_damage > 0) else 0.0
            f["ko_prize_value"] = f["opp_active_prize"] if f["can_ko_opp_now"] else 0.0
    else:
    ...
    # --- deck-specific tactical arithmetic ---
    f["powerful_hand_online"] = 1.0 if (myA and myA.get("id") == 743 and f.get("active_n_energy", 0) >= 1) else 0.0
    f["powerful_hand_damage"] = 20.0 * f["hand_size"] if f["powerful_hand_online"] else 0.0
    f["powerful_hand_cards_needed"] = (float((int(f["opp_active_hp"]) + 19) // 20)
                                           if f["powerful_hand_online"] and f["opp_active_hp"] > 0 else 0.0)
    f["powerful_hand_surplus"] = (f["hand_size"] - f["powerful_hand_cards_needed"]
                                   if f["powerful_hand_online"] else 0.0)
    opp_meta = cf(opA.get("id")) if opA else {}
    f["opp_active_retreat"] = float(opp_meta.get("retreat", opp_meta.get("retreatCost", 0)) or 0)
    f["opp_active_n_energy"] = float(len(_attached_letters(opA))) if opA else 0.0
    f["opp_active_retreat_gap"] = max(0.0, f["opp_active_retreat"] - f["opp_active_n_energy"])
    ...
    f["safe_pre_attack_options"] = 0.0
    try:
        import deck_policy_v3 as DP3
        ko = DP3.best_ko_attack(obs)
        if ko is not None and not ko[1].get("game_win"):
            f["safe_pre_attack_options"] = float(len(DP3.safe_pre_attack_indices(obs, ko[0])))
    except Exception:
        pass
    return f
