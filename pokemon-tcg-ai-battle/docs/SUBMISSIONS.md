...
| sub_starmie2.tar.gz (2026-06-25, post-audit) | **(uploading)** | Ready | POST-AUDIT build (commit 20dd7c6): the Model-B forensic audit fixed 6 confirmed bugs on top of the heuristic agent -- (A) search now samples top-30 meta opponent decks instead of assuming a mirror (opponent_meta_v1.json bundled), (B) Boss gust gated on no-active-KO (was re-targeting the KO), (C) Wally gated likewise (was stripping attack energy), (F) Ignition counted as 3 energy units, (G) tutor resolves by option.area, (J) no-suicide only suppresses self-removing abilities. Verified 0 errors; imitation flat (no regression); 4 frozen tests pass; field A/B alakazam 80% / denpa92 98%. THIS is the build to upload. (Prior pre-audit row below is superseded.) |
| sub_starmie2.tar.gz (2026-06-25, pre-audit) | (superseded) | Superseded | **HEURISTIC-FIRST STARMIE** (imitation-grounded upgrade of sub_starmie). Cinderace/Mega-Starmie deck piloted by `starmie_heuristics.agent`: forces the high-confidence mechanical moves -- evolve Staryu->Mega Starmie (not a 3rd), energy onto the line, KOs Jetting-Blow-first (conserve Ignition/Nebula), gust ANY-prize bench KO, Wally's only in KO range (+veto on pointless full-HP Wally's), Hero's Cape, Cinderace->Mega retreat-pivot, go-first, no-suicide, tutor/fetch targets (effect.id-keyed: Mega Signal->Mega Starmie, Poffin->Staryu, Turbo Flare->Water), Jetting snipe + gust target selection (weakness-aware), Crushing Hammer disruption -- and DEFERS ambiguous trainer/chip choices to search_v3; legal-first last. Heuristics derived from imitation-gap + LOSS analysis vs top pilots (tools/imitation_gap_v1.py, starmie_loss_capture_v1.py), NOT guessed; every rule measured. Loss analysis found the failure mode = empty bench (54% of losing-game decisions had bench 0; 0 missed KOs) -> added bench development (Poffin/Staryu when in-play<=2) and reordered to do all FREE development before the turn-ending attack. Agreement with top pilots 49.4% (deployed) -> 57.9% (evolve 76, select_card 67, play 27->40, go-first 86). Local A/B: field vs alakazam 80%, vs denpa92 85%; mirror vs deployed ~55% (the bench fix also stopped the empty-board collapse that made earlier mirror reads swing 37/58/38). Verified 0 errors, fast. Ladder-test vs sub_starmie's score is the real comparison. |
