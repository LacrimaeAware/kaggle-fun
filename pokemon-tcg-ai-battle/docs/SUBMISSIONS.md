...
| sub_starmie2.tar.gz (2026-06-25) | (built, ready) | Ready | **HEURISTIC-FIRST STARMIE** (imitation-grounded upgrade of sub_starmie). Cinderace/Mega-Starmie deck piloted by `starmie_heuristics.agent`: forces the high-confidence mechanical moves -- evolve Staryu->Mega Starmie (not a 3rd), energy onto the line, KOs Jetting-Blow-first (conserve Ignition/Nebula), gust ANY-prize bench KO, Wally's only in KO range (+veto on pointless full-HP Wally's), Hero's Cape, Cinderace->Mega retreat-pivot, go-first, no-suicide, tutor/fetch targets (effect.id-keyed: Mega Signal->Mega Starmie, Poffin->Staryu, Turbo Flare->Water), Jetting snipe + gust target selection (weakness-aware), Crushing Hammer disruption -- and DEFERS ambiguous trainer/chip choices to search_v3; legal-first last. Heuristics derived from imitation-gap analysis vs top pilots (tools/imitation_gap_v1.py), NOT guessed; every rule measured. Agreement with top pilots 49.4% (deployed) -> 55.3% over 40 winning games (evolve 25->69, energy 30->59, go-first 55->86, select_card 63->66, retreat 0->60). Local A/B (catastrophe check, NOT ladder-predictive, noise-dominated -- mirror read 37/58/38 across versions): crushes the field (vs alakazam 78%, vs denpa92 95%); same-deck mirror vs deployed swings widely (least informative test). Verified 0 errors, fast. Ladder-test vs sub_starmie's score is the real comparison. |
