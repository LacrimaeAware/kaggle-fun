...
USE_DYNAMIC_ATTACKS = False  # v3 agent enables this; legacy agent_search stays byte-behavior-compatible.
...
# Sprint A/B (tools/search_sprint.py): real-budget setup vs aggro 0.583 [0.457,0.699] n=60, and the
# confound-removed cont-clean (fixed N=4, no cap) 0.480 [0.348,0.615] n=50. BOTH ties straddling 0.5 ->
# the continuation policy makes no reliable difference; "aggro" kept. Continuation lever is closed.
...
def _selection_from_observation(value):
    """Accept either an Observation or a Select object (backward compatible)."""
...
def _rollout_pick(observation, is_me: bool = False) -> list:
    ...
    if k == 1 and n > 0:
        ...
        for i, option in enumerate(opts):
            if getattr(option, "type", None) != ATTACK_OPT:
                continue
            if USE_DYNAMIC_ATTACKS:
                try:
                    import deck_policy_v3 as DP3
                    value = DP3.attack_profile(option, observation)["value"]
                except Exception:
                    value = float(ATK.get(str(getattr(option, "attackId", None)), {}).get("d", 0) or 0)
            else:
                value = float(ATK.get(str(getattr(option, "attackId", None)), {}).get("d", 0) or 0)
            if value > best_value:
...
def _search(obs: dict, deck: list, time_budget: float = DEFAULT_BUDGET, leaf_mode: str = "hand",
    ...
    if allowed_indices is None:
        candidate_indices = list(range(len(opts)))
    else:
        candidate_indices = []
        seen = set()
        for raw in allowed_indices:
            try:
                index = int(raw)
            except Exception:
                continue
            if 0 <= index < len(opts) and index not in seen:
                candidate_indices.append(index)
                seen.add(index)
        if not candidate_indices:
            return None, None
    sample_count = int(n_determ if n_determ is not None else N_DETERM)
    if sample_count <= 0:
        return None, None
    ...
    complete_worlds = 0
    t0 = time.time()
    for world_index in range(sample_count):
        ...
        complete = True
        try:
            ordered = [i for i in _ordered_indices(nn, option_order) if i in candidate_indices]
            # Rotate a complete world's first option so no candidate systematically
            # receives the freshest engine state or the earliest timing slot.
            if ordered:
                shift = world_index % len(ordered)
        ...
        if complete and all(i in world_values for i in candidate_indices):
            for i in candidate_indices:
                sums[i] += world_values[i]
                counts[i] += 1
            complete_worlds += 1
        elif require_complete_world:
            # Partial worlds create option-order bias.  Discard them entirely.
            break

    if complete_worlds <= 0 or not any(counts[i] for i in candidate_indices):
        return None, None
...
def _rollout_topk(observation, k: int) -> list:
    """Top-k opponent choices by dynamic attack value; fallback to baseline pick."""
    sel = _selection_from_observation(observation)
    if (sel.maxCount or 0) != 1:
        return [_rollout_pick(observation)]
    scored = []
    for i, option in enumerate(sel.option):
        if getattr(option, "type", None) == ATTACK_OPT:
            if USE_DYNAMIC_ATTACKS:
                try:
                    import deck_policy_v3 as DP3
                    score = DP3.attack_profile(option, observation)["value"]
                except Exception:
                    score = float(ATK.get(str(getattr(option, "attackId", None)), {}).get("d", 0) or 0)
            else:
                score = float(ATK.get(str(getattr(option, "attackId", None)), {}).get("d", 0) or 0)
        else:
            score = -1.0
    ...
    return [[i] for _, i in scored[:max(1, k)]] or [_rollout_pick(observation)]
...
def _greedy_to_me(A, st, me):
    ...
    for _ in range(DEPTH_CAP):
        ...
        if cur is not None and cur.yourIndex == me:
            break
        st = A.search_step(st.searchId, _rollout_pick(st.observation))
    return st
...
def _simulate2(A, root_id, first_choice: int, me: int, leaf_mode: str = "hand", opp_k: int = 2):
    ...
    for _ in range(DEPTH_CAP):
        ...
        st = A.search_step(st.searchId, _rollout_pick(st.observation, is_me=True))   # finishing MY turn
    else:
        return _leaf_val(st, me, leaf_mode)
    opp_node, opp_sel = st.searchId, st.observation.select
    vals = []
    for choice in _rollout_topk(st.observation, opp_k):
        try:
