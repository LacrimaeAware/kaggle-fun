...
# EnergyType int (obs) -> cost token. Rainbow(10) is wildcard; Team Rocket (11)
# provides Psychic or Darkness, not every type.
...
def energy_shortfall(cost: list[str], attached: list[str]) -> int:
    ...
    for c in cost:
        if c == "C":
            ...
            avail[c] -= 1
        elif c in ("P", "D") and psychic_dark > 0:
            psychic_dark -= 1
        elif wild > 0:
            wild -= 1
        else:
            short += 1
    pool = sum(v for v in avail.values() if v > 0) + wild + psychic_dark
...
def _effective_damage(attacker: dict, defender: dict | None, attack: dict, hand_size: int = 0) -> float:
    name = str(attack.get("n") or attack.get("name") or "").lower()
    if attacker.get("id") == 743 and ("powerful hand" in name or not attack.get("dmg")):
        return float(max(0, 20 * hand_size))  # damage counters ignore weakness/resistance
    dmg = float(attack.get("dmg", 0) or 0)
    if defender:
        ac, dc = cf(attacker.get("id")), cf(defender.get("id"))
...
def _best_affordable(pkmn: dict, defender: dict | None = None, hand_size: int = 0):
    """(best affordable effective damage, min shortfall) for an in-play Pokemon."""
    c = cf(pkmn.get("id"))
    attached = _attached_letters(pkmn)
    best_dmg, min_short = 0.0, 99
    for a in c.get("atks", []):
        sh = energy_shortfall(a.get("cE", []), attached)
        min_short = min(min_short, sh)
        if sh <= 0:
            best_dmg = max(best_dmg, _effective_damage(pkmn, defender, a, hand_size))
...
def encode_state(obs: dict, perspective: int | None = None) -> dict:
    cur = obs.get("current") or {}
    players = cur.get("players") or []
    yi = cur.get("yourIndex", 0) if perspective is None else perspective
    ...
    if myA:
        dmg, short = _best_affordable(myA, opA, int(f["hand_size"] or 0))
    ...
    # --- opponent immediate threat, from public board and energy only ---
    if opA:
        opp_hand = int(opp.get("handCount", len(opp.get("hand") or [])) or 0)
        odmg, oshort = _best_affordable(opA, myA, opp_hand)
        f["opp_active_energy_short"] = oshort
        f["opp_active_affordable_dmg"] = odmg
        f["opp_can_ko_my_active_now"] = 1.0 if (myA and odmg >= myA.get("hp", 0) and odmg > 0) else 0.0
        f["my_active_survival_margin"] = (myA.get("hp", 0) - odmg) if myA else 0.0
    else:
        f["opp_active_energy_short"] = 99
        f["opp_active_affordable_dmg"] = 0.0
        f["opp_can_ko_my_active_now"] = 0.0
        f["my_active_survival_margin"] = f["my_active_hp"]
