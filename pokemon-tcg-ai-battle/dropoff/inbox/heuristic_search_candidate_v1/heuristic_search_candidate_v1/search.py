...
def _rollout_pick(observation, is_me: bool = False) -> list:
    """Coherent continuation policy.

    Deck-specific tutor, gust, evolution, recovery and multi-select choices are delegated to
    deck_policy. Search still evaluates the root option and remains final authority.
    """
    try:
        choice = DP.rollout_choice(observation, is_me=is_me)
    ...
    if k == 1 and n > 0:
        ...
        for i, o in enumerate(opts):
            if getattr(o, "type", None) == ATTACK_OPT:
                try:
                    d = DP.attack_damage(o, observation)
                except Exception:
...
def _rollout_topk(observation, k: int) -> list:
    """Top-k opponent replies, scored with dynamic attack damage and coherent target selection."""
    sel = observation.select
    ...
    for i, o in enumerate(opts):
        if getattr(o, "type", None) == ATTACK_OPT:
            try:
                d = DP.attack_value(o, observation)
            except Exception:
                d = ATK.get(str(getattr(o, "attackId", None)), {}).get("d", 0) or 0
        else:
            d = -1.0
    ...
    return top or [_rollout_pick(observation)]
...
def _greedy_to_me(A, st, me):
    ...
    for _ in range(DEPTH_CAP):
        ...
        if cur is not None and cur.yourIndex == me:
            break
        st = A.search_step(st.searchId, _rollout_pick(st.observation, is_me=False))
    return st
...
def _simulate2(A, root_id, first_choice: int, me: int, leaf_mode: str = "hand", opp_k: int = 2):
    ...
    for _ in range(DEPTH_CAP):
        ...
        return _leaf_val(st, me, leaf_mode)
    opp_node, opp_obs = st.searchId, st.observation
    vals = []
    for choice in _rollout_topk(opp_obs, opp_k):
        try:
